...
Dans mon dernier expérience, j'ai participé à la mise en œuvre de CI/CD via Azure pipeline, qui permet d'automatiser le processus de déploiement. Je suis très intéresse par CI/CD, donc mon attention se porte aussi sur Jenkins, c'est une package de technologie très populaire du marché. Donc je configure un serveur chez moi, puis essayer de déployer les applications via docker, et aussi  mes projets personnels
...
Mon travail consiste à construire et maintenir un système de gestion des données dans le secteur de la restauration. On a plusieurs projets intégrés:
